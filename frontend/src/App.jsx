...
function App() {
  ...
  return (
    <div className="skeuo-bg font-body text-slate-700 selection:bg-blue-200 min-h-screen antialiased">
      {/* Top Navigation Bar */}
      <nav className="fixed top-0 w-full z-50 bg-[#e0e5ec]/90 backdrop-blur-md shadow-sm border-b border-white/50 flex justify-between items-center px-10 h-20">
        ...
        <div className="flex items-center gap-6">
          <button className="skeuo-btn w-12 h-12 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors">
            <span className="material-symbols-outlined text-xl">notifications</span>
          </button>
          
          {user ? (
            <div className="flex items-center gap-6 pl-6 border-l border-white/50">
              <button onClick={() => setView('settings')} className="flex items-center gap-4 cursor-pointer group">
                <div className="skeuo-btn w-12 h-12 p-1 overflow-hidden transition-transform group-hover:scale-105">
              ...
              <button 
                ...
                className="skeuo-inner w-10 h-10 flex items-center justify-center text-slate-400 hover:text-red-500 transition-all rounded-xl"
                title="Logout"
              >
            ...
            <div className="flex items-center gap-4 pl-6 border-l border-white/50">
              <button onClick={() => setView('login')} className="skeuo-btn-primary px-10 py-4 text-xs font-black uppercase tracking-[0.2em] shadow-lg transition-all active:scale-95">
                Portal Access
              </button>
            </div>
          )}
      ...
      <main>
        ...
        <section className="relative min-h-[921px] flex items-center justify-center px-6 py-20 overflow-hidden bg-surface-container-low">
          ...
          <div className="container mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
            ...
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-fixed mb-6">
              ...
              <h1 className="text-6xl md:text-7xl font-headline font-extrabold text-on-surface tracking-tighter leading-[1.1] mb-6">
              ...
              <div className="flex flex-wrap gap-4">
                <button 
                  ...
                  className="skeuo-btn-primary px-10 py-5 font-headline font-bold text-lg active:scale-95 transition-all">
                  {user?.role === 'Patient' ? 'My Health Records' : 'Start AI Prediction'}
                </button>
                {(user?.role !== 'Patient') && (
                  <button 
            ...
            <div className="relative">
              ...
              <div className="skeuo-card absolute bottom-8 -left-8 md:-left-16 p-8 rounded-2xl max-w-sm border-white/40">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 rounded-xl skeuo-inner flex items-center justify-center text-blue-600">
                    <span className="material-symbols-outlined" data-weight="fill" style={{fontVariationSettings: "'FILL' 1"}}>verified</span>
                  </div>
                  <div>
                    <h4 className="font-headline font-bold text-slate-800">98.4% Accuracy</h4>
                    <p className="text-xs text-slate-500 font-medium tracking-tight">Verified Isolate Precision</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="h-4 w-full skeuo-inner rounded-full overflow-hidden p-1">
                    <div className="h-full bg-blue-500 rounded-full w-[98%] shadow-[inset_0_1px_2px_rgba(0,0,0,0.2)]"></div>
                  </div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                    Local Clinical Validation Peak
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="py-24 px-6 bg-surface">
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10 auto-rows-[300px]">
              {/* Bento Item 1 */}
              <div className="md:col-span-8 skeuo-card rounded-3xl p-10 flex flex-col justify-between group transition-all">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[10px] font-bold text-blue-600 px-4 py-1.5 skeuo-inner uppercase tracking-[0.2em]">Core Analytics</span>
                </div>
                <div>
                  <h3 className="text-3xl font-headline font-extrabold mb-3 text-slate-800">Molecular Pathogen Profiling</h3>
                  <p className="text-slate-500 max-w-md text-lg">Real-time identification of bacterial strains and genetic resistance markers through PCR data ingestion.</p>
                </div>
              </div>
              {/* Bento Item 2 */}
              <div className="md:col-span-4 skeuo-btn-primary rounded-3xl p-10 flex flex-col justify-between relative overflow-hidden">
                <div className="relative z-10">
                  <span className="material-symbols-outlined text-5xl mb-6">clinical_notes</span>
                  <h3 className="text-2xl font-headline font-bold mb-3">Patient History Link</h3>
                  <p className="opacity-80">Automated EMR cross-referencing for past allergies and prior exposures.</p>
                </div>
                <div className="absolute bottom-[-30px] right-[-30px] opacity-10">
                  <span className="material-symbols-outlined text-[150px]" data-weight="fill" style={{fontVariationSettings: "'FILL' 1"}}>history</span>
                </div>
              </div>
              {/* Bento Item 3 */}
              <div className="md:col-span-4 skeuo-card rounded-3xl p-10 flex flex-col justify-between">
                <div>
                  <div className="skeuo-inner w-16 h-16 rounded-2xl flex items-center justify-center mb-6">
                    <span className="material-symbols-outlined text-4xl text-blue-600">shield_with_heart</span>
                  </div>
                  <h3 className="text-2xl font-headline font-bold mb-3 text-slate-800">Stewardship First</h3>
                </div>
                <p className="text-slate-500 font-medium">Algorithmic bias towards narrow-spectrum agents to combat global resistance.</p>
              </div>
              {/* Bento Item 4 */}
              <div className="md:col-span-8 skeuo-card rounded-3xl p-10 grid md:grid-cols-2 gap-10 items-center">
                <div>
                  <h3 className="text-3xl font-headline font-extrabold mb-4 text-slate-800 tracking-tight">Instant Lab Integration</h3>
                  <p className="text-slate-500 mb-6 font-medium">Sync directly with hospital laboratory systems for sub-second data processing.</p>
                  <ul className="space-y-3">
                    <li className="flex items-center gap-3 text-sm font-bold text-slate-600">
                      <span className="material-symbols-outlined text-green-500 skeuo-inner p-1 rounded-full text-xs">check</span> HL7/FHIR Compliant
                    </li>
                    <li className="flex items-center gap-3 text-sm font-bold text-slate-600">
                      <span className="material-symbols-outlined text-green-500 skeuo-inner p-1 rounded-full text-xs">check</span> E2E Encryption
                    </li>
                  </ul>
                </div>
                <div className="skeuo-inner rounded-3xl p-8 h-full flex items-center justify-center group">
                  <span className="material-symbols-outlined text-8xl text-slate-300 group-hover:text-blue-400 transition-colors">api</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-20 px-6 bg-surface-container-low border-y border-outline-variant/10">
          <div className="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
            <div>
              <div className="text-5xl font-headline font-extrabold text-primary mb-2">2.4M</div>
              <div className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Validated Cases</div>
            </div>
            <div>
              <div className="text-5xl font-headline font-extrabold text-tertiary mb-2">18%</div>
              <div className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Cost Reduction</div>
            </div>
            <div>
              <div className="text-5xl font-headline font-extrabold text-on-surface mb-2">450+</div>
              <div className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Clinics Active</div>
            </div>
            <div>
              <div className="text-5xl font-headline font-extrabold text-error mb-2">&lt;2s</div>
              <div className="text-xs uppercase tracking-widest font-bold text-on-surface-variant">Inference Time</div>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer className="bg-[#e0e5ec] border-t border-white/40 py-24 px-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 items-center">
          ...
          <div className="flex justify-center md:justify-end gap-6">
            <button className="skeuo-btn w-14 h-14 flex items-center justify-center text-slate-400 hover:text-blue-600">
